...
if "${_ERROR}" ; then
    ...
    echo "${_COPYLEFT}" >&2
    echo "Usage: $(basename $0) [-n|--name PKGNAME] [-p|--packager 'FULLNAME <EMAIL>'] [-l|--log CHANGELOG] [-s|--set SETTINGS] [-C|--compat] ARCHIVE" >&2
    exit 1
fi
...
function _initial_variables {
    ...
    [ -z "${_PACKAGER}" -o "${_PACKAGER}" = " <>" ] && _PACKAGER="$(getent passwd ${USER}|cut -d: -f5|cut -d, -f1) <${USER}@$(hostname)>"
...
function _set_scripts {
    if [ "${_TOOLCHAIN}" = bootstrap ] ; then
        ...
        _BUILDMAKE="cargo build --release"
