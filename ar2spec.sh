...
if ! [ "${_ERROR}" = false -a -f "${_FILE}" ] ; then
    echo "AR2SPEC: Generating .spec file from software archive" >&2
    echo "Usage: $(basename $0) [-n|--name NAME] [-p|--packager \"FULLNAME <EMAIL>\"] [-l|--log LOG] [-b|--buildset SET] [-C|--compat] ARCHIVE" >&2
    exit 1
fi
...
function _set_attributes {
    ...
    if [ -z "${_VERSION}" ] ; then
        if [ "${_PKGNAME%[0-9]*}" != "${_PKGNAME}" ] ; then
            _VERSION="${_PKGNAME##*[A-Za-z]}"
            _PKGNAME="${_PKGNAME/${_VERSION}/}"
        else
            _VERSION="0"
        fi
    fi
    _PKGNAME="$(echo ${_PKGNAME}|sed 's|^[~0-9]*-||')"
    [ -z "${_NAME}" ] && _NAME="${_PKGNAME,,}"
    _SRCNAME=${_FILE/${_NAME}/%\{name\}}
    ...
    function _url_sourceforge {
        ...
        _SUMMARY=$(curl -s "${_URL}/"|grep -im1 '<meta name="description" content="Download'|sed 's|<meta name="description" content="Download.*for free\.[ \r]*||'|sed 's|" />||'|sed 's|\..*||')
...
function _enter_directory {
    ...
    for f in COPYING COPYING.L* LICENSE AUTHORS NEWS CHANGELOG ChangeLog README TODO THANKS *.pdf *.rst *.md *.txt ; do
        if [ -f "$f" -a "$f" != CMakeLists.txt -a "$f" != meson_options.txt ] ; then
            [ "$f" = "${f/ /}" ] && _DOCS+=" $f" || _DOCS+=" \"$f\""
        fi
    done
    if [ -n "${_DOCS}" ] ; then
        if grep -qs "GNU Lesser General Public License" ${_DOCS} ; then
            _LICENSE="LGPL"
    ...
    function _check_toolchain {
        if [ -f bootstrap.sh ] ; then
            ...
            [ -d 16x16 ] && _TOOLCHAIN="icons" || _TOOLCHAIN="themes"
...
function _set_scripts {
    if [ "${_TOOLCHAIN}" = bootstrap ] ; then
        ...
        _BUILDCONF="cmake .||%{cmake}"
        _BUILDMAKE="make||%{cmake_build}"
        _INSTALL="%{make_install}||%{cmake_install}"
    elif [ "${_TOOLCHAIN}" = qmake5 ] ; then
        _BUILDREQUIRES+=" qt5-qtbase-devel"
        _BUILDCONF="qmake-qt5 %{name}.pro||qmake-qt5"
        _BUILDMAKE="make||%{make_build}"
        _INSTALL="%{make_install}"
    elif [ "${_TOOLCHAIN}" = qmake4 ] ; then
        _BUILDREQUIRES+=" qt4-devel"
        _BUILDCONF="qmake-qt4 %{name}.pro||qmake-qt4"
        _BUILDMAKE="make||%{make_build}"
        _INSTALL="%{make_install}"
        ...
        _BUILDMAKE="cargo build --release||%{cargo_build}"
        _INSTALL="cargo install --root=%{buildroot}%{_prefix} --path=.||install -Dm755 target/release/%{name} %{buildroot}%{_bindir}/%{name}||%{cargo_install}"
        ...
        _INSTALL="SCRIPT=(*.{py,pl,lua,tcl})\ninstall -d %{buildroot}%{_datadir}/%{name}\ninstall -m755 \$SCRIPT %{buildroot}%{_datadir}/%{name}"
    elif [ "${_TOOLCHAIN}" = icons ] ; then
        _INSTALL="install -d %{buildroot}%{_datadir}/icons/${_SUBDIR:-$_NAME}\ncp -a * %{buildroot}%{_datadir}/icons/${_SUBDIR:-$_NAME}"
    elif [ "${_TOOLCHAIN}" = themes ] ; then
...
function _output_data {
    ...
    echo '%prep'
    echo '%setup' "${_SETUP}"
    "${_COMPAT}" && echo "if [ -d .git ];then git submodules update --init --recursive;fi"
    echo
    echo '%build'
    [ -n "${_SUBDIR}" ] && echo 'cd' "${_SUBDIR}"
    "${_COMPAT}" && echo "export CFLAGS+=' -Wno-error -fPIC -fPIE -Wno-format-security -fno-strict-aliasing -Wl,--allow-multiple-definition -Wno-narrowing -pipe -lm -lX11 -I/usr/include/qt5 ' CXXFLAGS+=' -fpermissive '"
    [ -n "${_BUILDSET}" ] && echo -e "${_BUILDSET}"
    "${_COMPAT}" && echo "cp -f /usr/lib/rpm/redhat/config.* ."
    [ -n "${_BUILDCONF}" ] && echo -e "${_BUILDCONF}"
    "${_COMPAT}" && echo "if find . -name '[Mm]akefile';then sed -i -e 's|-Wall|-Wall -Wno-error -fPIC -fPIE -fno-strict-aliasing -Wl,--allow-multiple-definition -fpermissive -pipe -lm -lX11 -I/usr/include/qt5|' -e 's|-Werror[=a-z\-]* | |' \`find . -name '[Mm]akefile'\`;fi"
    [ -n "${_BUILDMAKE}" ] && echo -e "${_BUILDMAKE}"
    echo
    echo '%install'
    [ -n "${_SUBDIR}" ] && echo 'cd' "${_SUBDIR}"
